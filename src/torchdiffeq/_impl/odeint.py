...
def odeint(func, y0, v, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    ...
    Args:
        ...
        y0: N-D Tensor giving starting value of `y` at time point `t[0]`. Optionally, `y0`
            can also be a tuple of Tensors.
        v: N-D Tensor giving all values of `v` ar time points `t`.
    ...
    solver = SOLVERS[method](func=func, y0=y0, v=v, rtol=rtol, atol=atol, **options)
