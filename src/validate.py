...
if __name__ == '__main__':
    ...
    with pbar:
        ...
        for i, j in zip(index[:-1], index[1:]):
            ...
            if (df.iloc[0].ind == df.iloc[1].ind) & 10 <= dur <= 11:
                ...
                mask = (syn.ind == ind) & (syn.datetime >= t0) & (syn.datetime <= t1)
                data = syn[mask][['t2m', 'td2m', 'ff', 'R12', 'phi', 'air', 'soilw', 'precip']]

                new_data = pd.DataFrame([])
                new_data.loc[:, 'ind'] = [ind]*81
                new_data.loc[0, 'datetime'] = t0
                new_data.loc[80, 'datetime'] = t1
                new_data.loc[:, 'datetime'] = new_data.datetime.interpolate(method='ffill')
    ...
    new_agro.drop_duplicates().to_parquet('data/new_agro.parquet')
